...
class AppStore extends Component {
  ...
  gettabApps = () => {
    ...
    const modifiedAppList = appsList.filter(
      eachApp => eachApp.category === ActiveTabId,
    )
    return modifiedAppList
  }

  getSearedApps = tabAppsList => {
    const {searchInput} = this.state
    const modifiedAppsList = tabAppsList.filter(eachApp =>
  ...
  render() {
    const {ActiveTabId} = this.state
    const tabAppsList = this.gettabApps()
    const serchedAppsList = this.getSearedApps(tabAppsList)
    return (
      <div className="app-container">
        <div className="app-bg-containe">
